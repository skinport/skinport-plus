...
const PUBLIC_API_KEY = process.env.WEBTRANSLATEIT_PUBLIC_API_KEY || 'proj_readonly_rgfBdjfA-x7HAN1OG37b3Q';
...
(async () => {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/projects/${PUBLIC_API_KEY}/zip_file`);


        if (response.status !== 200) throw new Error(`Status code is ${response.status}`);
        ...
        await async.eachSeries(entries, async (zipEntry) => {
            ...
            await fs.writeFile(
                ...
                JSON.stringify(JSON.parse(zipEntry.getData()
                    .toString('utf8'))),
            );
